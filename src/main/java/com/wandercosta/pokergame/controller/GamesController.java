...
public class GamesController {
    ...
    public PokerGame closeGame(@PathVariable final String uuid) {
        final PokerGame game = gameService.getOpen(uuid);
        game.setClosedAt(Instant.now());
        return gameService.save(game);
    }

    @PostMapping(path = "{uuid}/votes", produces = "application/json")
    public ResponseEntity<PokerGameVote> voteGame(@PathVariable final String uuid,
        ...
        final PokerGame game = gameService.getOpen(uuid);
        final PokerGameVote vote = voteFactory.build(dto, voterUUID, game);

        voteService.findVote(uuid, voterUUID).ifPresent(existentVote -> vote.setUuid(existentVote.getUuid()));
